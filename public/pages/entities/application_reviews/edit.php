        ...
        <div class="col-md-8 offset-md-2">
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="/application_reviews/<?= $review->id ?>">Review #<?= $review->id ?></a></li>
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <form method="POST" action="/application_reviews/<?= $review->id ?>/update" class="needs-validation" novalidate>
                        <?= csrf_field() ?>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select <?= errors('application_id') ? 'is-invalid' : '' ?>"
                                ...
                                <?php foreach ($applications as $app): ?>
                                    <option value="<?= $app->id ?>" <?= old('application_id', $review->application_id) == $app->id ? 'selected' : '' ?>>
                                        Application #<?= $app->id ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select <?= errors('reviewed_by') ? 'is-invalid' : '' ?>"
                                ...
                                <?php foreach ($persons as $person): ?>
                                    <option value="<?= $person->id ?>" <?= old('reviewed_by', $review->reviewed_by) == $person->id ? 'selected' : '' ?>>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control <?= errors('review_notes') ? 'is-invalid' : '' ?>"
                                      id="review_notes"
                                      name="review_notes"
                                      rows="6"><?= old('review_notes', $review->review_notes) ?></textarea>
                        ...
                        <div class="alert alert-info">
                            <small>
                                ...
                                Last updated: <?= date('F d, Y h:i A', strtotime($review->updated_at)) ?>
                                (Version: <?= $review->version ?>)
                            </small>
                        </div>

                        <div class="d-flex justify-content-between">
                            <a href="/application_reviews/<?= $review->id ?>" class="btn btn-secondary">
