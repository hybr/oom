<?php
$pageTitle = "Popular Organization Designations";
$pageDescription = "Manage popular organization designations";
...
<div class="container-fluid">
    ...
    <div class="row mb-4">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h1 class="h3 mb-1">🏢 Popular Organization Designations</h1>
                    <p class="text-muted mb-0">Manage and organize popular designations across organizations</p>
                </div>
                <div class="d-flex gap-2">
                    <button type="button" class="btn btn-outline-primary" onclick="showFilters()">
                        <i class="fas fa-filter"></i> Filters
                    </button>
                    <button type="button" class="btn btn-primary" onclick="showAddDesignationModal()">
            ...
            <div class="card border-0 shadow-sm">
                <div class="card-body">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            <h6 class="card-subtitle mb-1 text-muted">Total Designations</h6>
                            <h4 class="card-title mb-0" id="totalDesignations">0</h4>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            ...
                            <h4 class="card-title mb-0" id="activeDesignations">0</h4>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            ...
                            <h4 class="card-title mb-0" id="functionalDesignations">0</h4>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="col-12">
            <div class="card border-0 shadow-sm">
                <div class="card-body">
                    <div class="row g-3">
                        <div class="col-md-3">
                            <label for="designationTypeFilter" class="form-label">Designation Type</label>
                            <select class="form-select" id="designationTypeFilter">
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="text" class="form-control" id="searchFilter" placeholder="Search designations...">
                        </div>
                    </div>
                    <div class="row mt-3">
                        <div class="col-12">
                ...
                <div class="table-responsive">
                    <table class="table table-hover mb-0">
                        <thead class="bg-light">
                            <tr>
                                <th class="border-0 ps-3">Designation</th>
    ...
    <div id="cardView" class="d-none">
        <div id="designationsCardContainer">
    ...
    <div id="treeView" class="d-none">
        <div class="card border-0 shadow-sm">
            <div class="card-body">
                <div id="designationTreeContainer">
                    <div class="text-center py-4">
                        <div class="spinner-border text-primary" role="status">
                            <span class="visually-hidden">Loading...</span>
                        </div>
            ...
            <div class="modal-body">
                <form id="designationForm">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-6">
                            <label for="designationCode" class="form-label">Code *</label>
                            <input type="text" class="form-control" id="designationCode" name="code" required>
                        </div>
                    </div>

                    <div class="row mb-3">
                        <div class="col-md-6">
                            <label for="designationType" class="form-label">Type *</label>
                            <select class="form-select" id="designationType" name="designation_type" required>
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-6">
                            <label for="parentDesignation" class="form-label">Parent Designation</label>
                            <select class="form-select" id="parentDesignation" name="parent_designation_id">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="designationDescription" name="description" rows="3"></textarea>
                    </div>
...
function switchToTableView() {
    ...
    displayDesignations(lastLoadedDesignations || []);
}
...
function switchToTreeView() {
    ...
    displayTreeView(lastLoadedDesignations || []);
}
...
async function loadDesignations() {
    try {
        ...
        const response = await fetch(`api/popular_organization_designations.php?${params}`);
        const data = await response.json();

        if (data.success) {
            updateStatistics(data.statistics);
            lastLoadedDesignations = data.designations;
            displayDesignations(data.designations);
            updatePagination(data.pagination);
        } else {
            console.error('Failed to load designations:', data.message);
            showAlert('Failed to load designations: ' + data.message, 'danger');
        }
    } catch (error) {
        console.error('Error loading designations:', error);
        showAlert('Error loading designations. Please try again.', 'danger');
    }
}

// Update statistics
function updateStatistics(stats) {
    document.getElementById('totalDesignations').textContent = stats.total || 0;
    document.getElementById('activeDesignations').textContent = stats.active || 0;
    document.getElementById('functionalDesignations').textContent = stats.functional || 0;
    document.getElementById('totalEmployees').textContent = stats.totalEmployees || 0;
}

// Display designations based on current view
function displayDesignations(designations) {
    if (currentView === 'table') {
        displayTableView(designations);
    } else if (currentView === 'card') {
        displayCardView(designations);
    }
}

// Display table view
function displayTableView(designations) {
    ...
    tbody.innerHTML = designations.map(dept => `
        <tr>
            <td class="ps-3">
                <div>
            ...
            <td>
                <span class="badge bg-secondary">${escapeHtml(dept.designation_type || 'N/A')}</span>
            </td>
            <td>
            ...
            <td>
                ${dept.parent_designation_name ? escapeHtml(dept.parent_designation_name) : '<span class="text-muted">Top Level</span>'}
            </td>
            <td class="text-end pe-3">
                <div class="btn-group btn-group-sm">
                    <button class="btn btn-outline-primary" onclick="editDesignation(${dept.id})" title="Edit">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button class="btn btn-outline-danger" onclick="deleteDesignation(${dept.id}, '${escapeHtml(dept.name)}')" title="Delete">
...
// Display card view
function displayCardView(designations) {
    const container = document.getElementById('designationsCardContainer');
    ...
    container.innerHTML = designations.map(dept => `
        <div class="card mb-3">
            <div class="card-body">
                ...
                <div class="d-flex gap-2">
                    <button class="btn btn-sm btn-outline-primary" onclick="editDesignation(${dept.id})">
                        <i class="fas fa-edit"></i> Edit
                    </button>
                    <button class="btn btn-sm btn-outline-danger" onclick="deleteDesignation(${dept.id}, '${escapeHtml(dept.name)}')">
...
// Display tree view
function displayTreeView(designations) {
    const container = document.getElementById('designationTreeContainer');
    ...
    const tree = buildTree(designations);
    container.innerHTML = renderTree(tree);
}

// Build tree structure from flat array
function buildTree(designations) {
    const map = {};
    const roots = [];

    // Create map
    designations.forEach(dept => {
        map[dept.id] = { ...dept, children: [] };
    });

    // Build tree
    designations.forEach(dept => {
        if (dept.parent_designation_id && map[dept.parent_designation_id]) {
            map[dept.parent_designation_id].children.push(map[dept.id]);
        } else {
...
function applyFilters() {
    currentFilters = {
        designation_type: document.getElementById('designationTypeFilter').value,
...
function clearFilters() {
    document.getElementById('designationTypeFilter').value = '';
...
function editDesignation(id) {
    // Load designation data and populate form
    fetch(`api/popular_organization_designations.php?action=get&id=${id}`)
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                const dept = data.designation;
                document.getElementById('designationModalLabel').textContent = 'Edit Designation';
                document.getElementById('designationId').value = dept.id;
                document.getElementById('designationName').value = dept.name || '';
                document.getElementById('designationCode').value = dept.code || '';
                document.getElementById('designationType').value = dept.designation_type || '';
                ...
                document.getElementById('parentDesignation').value = dept.parent_designation_id || '';
                document.getElementById('designationDescription').value = dept.description || '';
                ...
                loadParentDesignations(dept.id);
                new bootstrap.Modal(document.getElementById('designationModal')).show();
            } else {
                showAlert('Failed to load designation: ' + data.message, 'danger');
            }
        })
        .catch(error => {
            console.error('Error loading designation:', error);
            showAlert('Error loading designation. Please try again.', 'danger');
        });
}

function loadParentDesignations(excludeId = null) {
    fetch('api/popular_organization_designations.php?action=list&limit=1000')
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                const select = document.getElementById('parentDesignation');
                select.innerHTML = '<option value="">No Parent (Top Level)</option>';

                data.designations.forEach(dept => {
...
function saveDesignation() {
    ...
    const isEdit = document.getElementById('designationId').value;
    const url = isEdit ?
        `api/popular_organization_designations.php?action=update&id=${isEdit}` :
        'api/popular_organization_designations.php?action=create';
    ...
    .catch(error => {
        ...
        showAlert('Error saving designation. Please try again.', 'danger');
    });
}

function deleteDesignation(id, name) {
    if (confirm(`Are you sure you want to delete "${name}"?`)) {
        fetch(`api/popular_organization_designations.php?action=delete&id=${id}`, {
            method: 'POST'
        })
        .then(response => response.json())
        .then(data => {
        ...
        .catch(error => {
            ...
            showAlert('Error deleting designation. Please try again.', 'danger');
        });
    }
}
...
function updatePagination(pagination) {
    ...
    // Update info
    info.textContent = `Showing ${((pagination.currentPage - 1) * pagination.itemsPerPage) + 1}-${Math.min(pagination.currentPage * pagination.itemsPerPage, pagination.totalItems)} of ${pagination.totalItems} designations`;
